...
class UCB(IndexPolicy):
    ...
    def computeAllIndex(self,choice0,choice1,choice2):
        """ Compute the current indexes for all arms, in a vectorized manner."""
        time_choices = np.asarray([choice0, choice1, choice2])
        k = 0.05
        indexes = (self.rewards / self.pulls) + np.sqrt((2 * np.log(self.t)) / self.pulls)
        #indexes = (self.rewards / self.pulls) + np.sqrt((2 * np.log(self.t)) / self.pulls) + k * np.sqrt(time_choices)
