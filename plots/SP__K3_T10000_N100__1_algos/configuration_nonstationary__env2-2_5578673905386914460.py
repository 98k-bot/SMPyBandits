...
if __name__ == '__main__':
    print(
        "Warning: this script 'configuration_nonstationary.py' is NOT executable. Use 'main.py' or 'make single' ...")  # DEBUG
    exit(0)
...
configuration = {
    ...
    # XXX Default for non stationary: we do not have a better lower bound than Lai & Robbins's.
    # --- Arms
    "environment": [],
}
...
if 1 in PROBLEMS:
    configuration["environment"] += [
        {  # A simple piece-wise stationary problem
            ...
            "params": {
                "listOfMeans": [
                    [0.3, 0.4, 0.5],  # 0    to 399
                    [0.9, 0.4, 0.5],  # 400  to 799
                    [0.3, 0.4, 0.5],  # 800  to 1199
                    [0.9, 0.4, 0.5],  # 1200 to 1599
                    [0.3, 0.4, 0.5],  # 1600 to end
                ],
                "changePoints": [
                    int(0),
                    int(1000),
                    ...
                    int(7000),
                    #int(0 * HORIZON / 2000.0),
                    #int(3000 * HORIZON / 2000.0),
                    #int(5000 * HORIZON / 2000.0),
                    #int(1200 * HORIZON / 2000.0),
                    #int(1600 * HORIZON / 2000.0),
                    # 20000,  # XXX larger than horizon, just to see if it is a problem?
                ],
...
def check_condition_on_piecewise_stationary_problems(horizon, listOfMeans, changePoints):
    ...
    for m in range(M - 1):
        ...
        print("\nChecking m-th (m = {}) sequence, µ_m = {}, µ_m+1 = {} and tau_m = {} and tau_m+1 = {}".format(m, mus_m,
                                                                                                               mus_mp1,
                                                                                                               tau_m,
                                                                                                               tau_mp1))  # DEBUG
        for i, (mu_i_m, mu_i_mp1) in enumerate(zip(mus_m, mus_mp1)):
            ...
            print("   - For arm i = {}, gap = {:.3g} and length = {} with lowerbound on length = {}...".format(i, gap,
                                                                                                               length,
                                                                                                               lowerbound))  # DEBUG
            if length < lowerbound:
                print(
                    "WARNING For arm i = {}, gap = {:.3g} and length = {} < lowerbound on length = {} !!".format(i, gap,
                                                                                                                 length,
                                                                                                                 lowerbound))  # DEBUG
...
NB_BREAK_POINTS = max([len(env["params"]["changePoints"]) - (1 if 0 in env["params"]["changePoints"] else 0) for env in
                       configuration["environment"]])
...
TAUS = [
    # 500, 1000, 2000,
    int(2 * np.sqrt(HORIZON * np.log(HORIZON) / max(1, NB_BREAK_POINTS))),
    # "optimal" value according to [Garivier & Moulines, 2008]
]
#GAMMAS = [0.95]  # +
#
GAMMAS = [0.99]
...
WINDOW_SIZE = NB_ARMS * int(
    np.ceil(HORIZON / 100))  #: Default window size :math:`w` for the M-UCB and SW-UCB algorithm.
...
MIN_NUMBER_OF_OBSERVATION_BETWEEN_CHANGE_POINT = np.min(np.diff(CHANGE_POINTS)) // (2 * NB_ARMS) if len(
    CHANGE_POINTS) > 1 else HORIZON
...
print(
    "\nUsing Upsilon_T = {} break-points (time when at least one arm changes), and C_T = {} change-points (number of changes of all arms).".format(
        UPSILON_T, NUMBER_OF_CHANGE_POINTS))  # DEBUG
...
if len(CHANGE_POINTS) > 1 and len(PROBLEMS) == 1:  # and set(PROBLEMS) <= {1,2,3,4,5,6}:
    ...
    min_change_on_mean = min(delta for delta in
                             [min([delta for delta in np.abs(np.diff(np.array(LIST_OF_MEANS)[:, i])) if delta > 0]) for
                              i in range(np.shape(LIST_OF_MEANS)[1])] if delta > 0)
    print("min_change_on_mean =", min_change_on_mean)  # DEBUG
    min_optimality_gap = min(delta for delta in
                             [min([delta for delta in np.abs(np.diff(np.array(LIST_OF_MEANS)[j, :])) if delta > 0]) for
                              j in range(np.shape(LIST_OF_MEANS)[0])] if delta > 0)
...
DELTA_T_UpsilonT_K = 1.0 / np.sqrt(
    NB_ARMS * UPSILON_T * HORIZON)  # XXX tune the delta as just a function of T and Upsilon_T
DELTA_T_CT = 1.0 / np.sqrt(
    NUMBER_OF_CHANGE_POINTS * HORIZON)  # XXX tune the delta as just a function of T and Upsilon_T
...
ALPHA_T_UpsilonT = ALPHA_0 * np.sqrt(
    UPSILON_T * np.log(HORIZON) / HORIZON)  # XXX tune the alpha as just a function of T and Upsilon_T
ALPHA_T_UpsilonT_K = ALPHA_0 * np.sqrt(
    NB_ARMS * UPSILON_T * np.log(HORIZON) / HORIZON)  # XXX tune the alpha as just a function of T and Upsilon_T
ALPHA_T_CT = ALPHA_0 * np.sqrt(
    NUMBER_OF_CHANGE_POINTS * np.log(HORIZON) / HORIZON)  # XXX tune the alpha as just a function of T and Upsilon_T
