...
class Evaluator(object):
    ...
    def __init__(self, configuration,
        ...
        self.numberOfCPDetections = dict()
        self.tauPulls = dict()#: For each env, store the number of change-point detections by each algorithms, to print it's average at the end (to check if a certain Change-Point detector algorithm detects too few or too many changes).
        ...
        for envId in range(len(self.envs)):
            ...
            self.tauPulls[envId] = np.zeros((self.nbPolicies, self.envs[envId].nbArms, self.horizon), dtype=np.int32)
    ...
    def startOneEnv(self, envId, env):
        ...
        def store(r, policyId, repeatId):
            ...
            #self.tauPulls[envId][policyId,:,:] = 1 - self.allPulls[envId][policyId,:,:]
            action = 0
            for i in range(self.horizon):
                for j in range(self.envs[envId].nbArms):
                    if self.allPulls[envId][policyId,j,i] == 0:
                        self.tauPulls[envId][policyId,j,i] = self.allPulls[envId][policyId,j,i] + 1
                    elif self.allPulls[envId][policyId,j,i] == 1:
                        self.tauPulls[envId][policyId, j, i] = 0
                        action = j
                        #print('action', action)
            if hasattr(self, 'minCumRewards'):
                self.minCumRewards[policyId, envId, :] = np.minimum(self.minCumRewards[policyId, envId, :], np.cumsum(r.rewards)) if repeatId > 1 else np.cumsum(r.rewards)
    ...
    def getCumulatedRegret_LessAccurate(self, policyId, envId=0):
        ...
        #return np.cumsum(self.getRewards(policyId, envId))
    ...
    def getCumulatedRegret(self, policyId, envId=0, moreAccurate=True):
    ...
    def plotRegrets(self, envId=0,
        ...
        for i, policy in enumerate(self.policies):
            ...
            lw = 3 if ('$N=' in policy.__cachedstr__ or 'Aggr' in policy.__cachedstr__ or 'CORRAL' in policy.__cachedstr__ or 'LearnExp' in policy.__cachedstr__ or 'Exp4' in policy.__cachedstr__) else 8
...
def delayed_play(env, policy, horizon,
    ...
    for t in prettyRange:
        ...
        choice = policy.choice(choice0, choice1, choice2)
        if choice == 0:
            tau_choices = choice0
            choice0 = 0
            choice1 = choice1 + 1
        ...
        policy.getReward(choice, reward - 0.01 * tau_choices)
    ...
    if repeatId == 0 and hasattr(policy, 'estimatedOrder'):
        order = policy.estimatedOrder(choice0, choice1, choice2)
